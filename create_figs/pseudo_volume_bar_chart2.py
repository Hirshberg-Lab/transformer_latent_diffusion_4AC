...
def compute_metrics_vs_k(samples, mu, max_k=15, confidence_level=0.90,c=10):
    """
    Calculates the Pseudo-Volume as functions of k.
    """
    # 1. Setup
    ...
    # ==========================================
    #      Normalized Relative Geometric Volume
    # ==========================================
    ...
    for k in range(1, max_k + 1):
        log_sum = np.sum(np.log1p(widths[:k]/c)) # log(1+w1) + ... + log(1+wk)
        v_k = np.expm1(log_sum) # e^(log_sum) - 1
    ...
    return k_values, normalized_volumes
...
def main():
    ...
    gs_main = gridspec.GridSpec(2, 1, height_ratios=[2, 1.5], hspace=0.05)
    
    # Top row with large panels (metrics)
    gs_metrics = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=gs_main[0], wspace=0.5)
    # Bottom row with small wspace (spectra)
    gs_spectra = gridspec.GridSpecFromSubplotSpec(1, 5, subplot_spec=gs_main[1], wspace=0.29)
    
    axes = {}
    for i in range(5):
        axes[f's{i}'] = fig.add_subplot(gs_spectra[0, i])
    axes['v'] = fig.add_subplot(gs_metrics[0, 1])
    ...
    for i, idx in enumerate(pick_indices):
        ...
        mean_l2 = np.mean(np.linalg.norm(samples, axis=1))
        if mean_l2 > 0:
            normalized_samples = samples.copy()  / mean_l2
        else:
            normalized_samples = samples.copy()
        normalized_mu = normalized_samples.mean(axis=0)
        lower_percentile = (1.0 - confidence_level) / 2.0 * 100
        upper_percentile = (confidence_level + (1.0 - confidence_level) / 2.0) * 100
        
        centered_samples = normalized_samples - normalized_mu
        max_k=max(target_k_values)
        max_k = min(max_k, samples.shape[0] - 1, samples.shape[1])
        ...
        L_p = np.percentile(projections, lower_percentile, axis=0)
        U_p = np.percentile(projections, upper_percentile, axis=0)
        width_list.append( U_p - L_p )
    c = np.max(width_list)/2
    print(f'c={c}')
    print(f'min_width={np.min(width_list)}')
    ...
    for i, idx in enumerate(pick_indices):
        ...
        ax_s.plot(omega, ground_truth, '--',lw=3, color='k', label='GT' if i==0 else None)
        ax_s.plot(omega, mu, '-', color=colors[i], linewidth=3, label='Mean' if i==0 else None)
        [ax_s.plot(omega,samples[l,:], 
        color=colors[i], alpha=0.5,linewidth=0.8) for l in range(20)]
        ...
        k_values, normalized_volumes = compute_metrics_vs_k(normalized_samples, normalized_mu, max_k=max(target_k_values),c=c)
    ...
    ax_vol.set_ylabel("$\\mathcal{P}$",labelpad=-1.)
    ...
    out_path = BASE_DIR / 'figs/pseudo_volume_bar_chart2.pdf'
