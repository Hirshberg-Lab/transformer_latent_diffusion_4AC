...
def main():
    ...
    n_rows = 5
    n_cols = 6
    # Same figsize and layout as visualize_convergence.py
    ...
    for j in range(n_rows):
        ...
        for k_col in range(n_cols):
            ...
            if k == 0:
                ev = 0.0
                text_str = f'EV={ev:.0%}'
                # For k=0, plot the mean
                ax.plot(omega, mu, color='tab:blue', linewidth=2, label='Mean' if j == 0 and k_col == 0 else None)
            else:
                c_val = coeffs_p[k-1]
                ev = np.sum(pca.explained_variance_ratio_[:k])
                text_str = f'EV={ev:.0%}'
                ax.plot(omega, components[k-1], color='tab:blue', linewidth=2, label='PCA Comp.' if j == 0 and k_col == 1 else None)
    ...
    out_path = BASE_DIR / 'figs/visualize_components.pdf'
